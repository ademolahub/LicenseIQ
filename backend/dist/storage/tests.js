...
async function testTablesRoundTrip() {
    ...
    const tablesClient = new storage_1.MockTablesClient();
...
async function testBlobsRoundTrip() {
    ...
    const blobsClient = new storage_1.MockBlobsClient();
    ...
    const nonExistent = await blobsClient.getAssessment((0, uuid_1.v4)());
    if (nonExistent !== null) {
...
async function testQueueMessageId() {
    ...
    const queueClient = new storage_1.MockQueueClient();
    // Enqueue a message
    const assessmentId = (0, uuid_1.v4)();
    const tenantId = (0, uuid_1.v4)();
    ...
    if (dequeuedMessage.messageId !== messageId ||
